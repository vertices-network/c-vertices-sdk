...
static provider_info_t providers =
    {.url = (char *)"https://api.testnet.algoexplorer.io", .port = 0, .header = (char *)""};

// Alice's account is used to send data, private key is taken from config/private_key.bin
static account_info_t alice_account = {.public_b32 = {0}, .private_key = {0}, .amount = 0};
// Bob is receiving the money 😎
static account_info_t bob_account =
    {.public_b32 = "27J56E73WOFSEQUECLRCLRNBV3D74H7BYB7USEXCJOYPLBTACULABWMLVU", .private_key = {0}, .amount = 0};
...
vertices_evt_handler(vtc_evt_t *evt)
{
    ...
    {
        default:
            ESP_LOGI(TAG, "Unhandled event: %u", evt->type);
            break;
    }
...
void vtc_wallet_task(void *param)
{
...
void app_main(void)
{
    ...
    xTaskCreatePinnedToCore(&vtc_wallet_task, "vtc_wallet_task", 8192, NULL, 5, NULL, 1);
}
